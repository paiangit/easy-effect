...
const Draggable: FC<DraggableProps> = ({
  ...
  const getItemStyle = useCallback((
    ...
    const { left, top, width, height } = dragContainer.current?.getBoundingClientRect();
    const offsetX = Math.min(Math.max(position.x, left), left + width) - originPosition.cX;
    const offsetY = Math.min(Math.max(position.y, top), top + height) - originPosition.cY;
    ...
    switch (handleType) {
      ...
      case 'move':
        ...
        itemStyle.left = Math.max(0, Math.min(itemStyle.left, width - itemStyle.width));
        itemStyle.top = Math.max(0, Math.min(itemStyle.top, height - itemStyle.height));
        break;
      // 东
      case 'east':
        // 向右拖拽添加宽度
  ...
  // 鼠标被按下
  const onMouseDown = useCallback((dir: HandleType, e: React.MouseEvent<HTMLElement>) => {
    // 对于点击的不是鼠标左键的情况，不做处理
            ...
            <div className="draggable-item-child">{children}</div>
            {
              ...
              handles.map(item =>
                <div
                  ...
                  onMouseDown={(e: React.MouseEvent<HTMLElement>) => onMouseDown(item as HandleType, e)}
                  key={item}
                >
                </div>
              )
